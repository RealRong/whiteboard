...
长期最优不应该让 editor 在每次输入时都实时重新 exact fit。

原因很简单：

1. 输入一个字符会触发文本重排
2. 文本重排又会触发字号变化
3. 字号变化又会再次改变换行和 caret 位置
...
1. 展示态使用 auto-fit
2. 进入编辑时冻结当前展示字号
3. 整个编辑 session 内保持这个字号不变
4. 提交或退出编辑后，再根据最终文本重新做一次 finalize
...
这套策略不需要修改 document 模型，只需要在 runtime 的编辑 session 内保存一个冻结字号。
