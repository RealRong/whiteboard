...
import { getNodeAABB } from '../../geometry/geometry'
...
export const getCollapsedGroupIds = (nodes: Node[]) => {
  ...
  nodes.forEach((node) => {
    ...
    const collapsed = node.data && typeof node.data.collapsed === 'boolean' ? node.data.collapsed : false
    if (collapsed) {
      set.add(node.id)
    }
  })
...
export const expandGroupRect = (groupRect: Rect, contentRect: Rect, padding: number): Rect => {
