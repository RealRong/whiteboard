...
const orderedNodes = (documentAtom: PrimitiveAtom<Document>): Atom<Node[]> => {
...
const visibleNodes = (orderedNodesAtom: Atom<Node[]>): Atom<Node[]> => {
...
const canvasNodes = (visibleNodesAtom: Atom<Node[]>): Atom<Node[]> => {
...
const visibleEdges = (
  ...
  return atom((get) => {
    ...
    const nextVisibleEdges = ordered.length ? ordered : EMPTY_EDGES
...
const mindmapRoots = (visibleNodesAtom: Atom<Node[]>): Atom<NodeId[]> => {
  let visibleNodesRef: Node[] | undefined
...
const snapshot = ({
  ...
  return atom((get) => {
    ...
    if (
      ...
      && cache.indexes.visibleNodeIndexById === snapshotIndexes.visibleNodeIndexById
      && cache.indexes.canvasNodeIndexById === snapshotIndexes.canvasNodeIndexById
    ) {
      return cache
    }
