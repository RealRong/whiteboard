...
export function createPanZoom(domElement, options) {
  ...
  let initialX = typeof options.initialX === 'number' ? options.initialX : transform.x
  let initialY = typeof options.initialY === 'number' ? options.initialY : transform.y
  let initialZoom = typeof options.initialZoom === 'number' ? options.initialZoom : transform.scale
  ...
  function setTransform(nextTransform: any, duration: number = 500, skipAnimation?: boolean) {
    cancelZoomAnimation()
    if (skipAnimation) {
  ...
  function listenForEvents() {
    owner.addEventListener('pointerdown', onMouseDown, { passive: false })
  ...
  function releaseEvents() {
    ...
    owner.removeEventListener('pointerdown', onMouseDown)
